...
tdcli_function ({
    ...
    return tdcli.sendMessage(msg.to.id, msg.id, 0, "?? _ÊãÊ ÊÑŞíÉ ÌãíÚ ÇáãÔÑİíä Úáì ÇáãÌãæÚÉ æÃÕÈÍ ãäÔÆ ÇáãÌãæÚÉ ÇáÂä ãÇáß ÇáãÌãæÚÉ_", 0, "md")
 end
...
if string.len(user) > 250 or ctrl_chars > 250 or real_digits > 250 then
 return "??áä ÇÚÑÖ ÇáÇÓã áÇäå ÓÈÇã ??"
else
return user
end
else
...
function file_dl(file_id)
 ...
 function banned_list(chat_id)
  ...
  if not data[tostring(chat_id)] then
    ...
    return '??*  ÇáãÌãæÚå áíÓÊ ãÖÇİå*'
   
  end
  -- determine if table is empty
  if next(data[tostring(chat_id)]['banned']) == nil then --fix way

					return "??*  áÇíæÌÏ ÃÚÖÇÁ ãÍÙæÑíä İí åĞå ÇáãÌãæÚå*"
              
				end

   message = '??_  ŞÇÆãå ÇáÇÚÖÇÁ ÇáãÍÙæÑíä :_\n'
 ...
 function silent_users_list(chat_id)
  ...
  if next(data[tostring(chat_id)]['is_silent_users']) == nil then --fix way
					...
					return "??*  áÇíæÌÏ ÇÚÖÇÁ ãßÊæãíä İí åĞå ÇáãÌãæÚå*"
             end
				
   message = '_??  ŞÇÆãå ÇáÇÚÖÇÁ ÇáãßÊæãíä :_\n'
...
function whitelist(chat_id)
  ...
  if next(data[tostring(chat_id)]['whitelist']) == nil then --fix way
     ...
     return "??*  áÇíæÌÏ ÇÚÖÇÁ ããíÒíä Öãä ÇáŞÇÆãå İí åĞå ÇáãÌãæÚå*"
             
				end

   message = '_??  ŞÇÆãå ÇáÇÚÖÇÁ ÇáããíÒíä :_\n'
 ...
 function gbanned_list(msg)
  ...
  if next(data['gban_users']) == nil then --fix way
					return "??*  áÇíæÌÏ ÇÚÖÇÁ ãÍÙæÑíä ÚÇã*"
				end
   message = '_??  ŞÇÆãå ÇáãÍÙæÑíä ÚÇã :_\n'
 ...
 function filter_list(msg)
  ...
  if next(data[tostring(msg.chat_id_)]['filterlist']) == nil then --fix way
    ...
    return "_??  ŞÇÆãå ÇáßáãÇÊ ÇáããäæÚå İÇÑÛå_"
  ...
  if not data[tostring(msg.chat_id_)]['filterlist'] then
    ...
    save_data(_config.moderation.data, data)
       ...
       filterlist = '_??  ŞÇÆãå ÇáßáãÇÊ ÇáããäæÚå :_\n'
